#!/usr/bin/env python3
"""
Generate a self-contained HTML finance news screener.

Uses Claude (claude-sonnet-4-6) with web_search to fetch today's market intelligence:
...
SYSTEM_PROMPT = """You are a financial market intelligence analyst with real-time web search.
...
def _user_prompt() -> str:
    ...
    return (
        f"Today is {today}. Search for current market data and news. Find:\n"
...
def fetch_market_data(api_key: str) -> dict:
    ...
    print("Searching for market intelligence...", flush=True)
...
<head>
  ...
  <title>Market Intelligence Screener &mdash; {date_str}</title>
  <style>{CSS}</style>
</head>
<body>
<div class="container">
  ...
  <div class="header">
    ...
    <p class="header-meta">Generated {timestamp} &middot; Powered by Claude + Web Search</p>
  </div>
